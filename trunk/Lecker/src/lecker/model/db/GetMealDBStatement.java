...
public class GetMealDBStatement implements DBStatement<Meal> {
	...
	public GetMealDBStatement(String name) {
		try {
			...
			statementComments = Handler.getInstance().getDBManager().prepareStatement("SELECT * FROM " + DBManager.TITLE_COMMENT + " WHERE " + DBManager.TITLE_MEAL_NAME + "='" + name + "';");
			...
			statementDates = Handler.getInstance().getDBManager().prepareStatement("SELECT " + DBManager.TITLE_LABELING_LABEL + " FROM " + DBManager.TITLE_LABELING + " WHERE " + DBManager.TITLE_LABELING_MEAL + "='" + name + "';");
